...
export const parseViewport = (input) => {
    if (!input)
...
export const makeCaptureOptions = (input) => ({
...
export const runCaptureCommand = async (rawOptions, logger = console) => {
    const options = makeCaptureOptions(rawOptions);
    const startedAt = Date.now();
    try {
        const discovery = await discoverStories(options.url, { timeoutMs: options.timeout });
        const capture = await captureStories({ baseUrl: discovery.baseUrl, stories: discovery.stories, options });
        const exitCode = capture.stats.failed === capture.stats.total ? 1 : 0;
        const totalDuration = Date.now() - startedAt;
        logger.log(`vrt capture summary: total=${capture.stats.total} captured=${capture.stats.captured} failed=${capture.stats.failed} duration=${totalDuration}ms output=${path.resolve(options.outDir)}`);
