...
describe('discoverStories', () => {
  it('tries endpoints in order and succeeds on storybook-index.json after index.json 404', async () => {
    const fetchMock = vi
      ...
      .mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: async () => storybookIndexFixture,
      } as Response);

    const result = await discoverStories('http://localhost:6006', {
      fetchImpl: fetchMock,
    });
  ...
  it('throws when all endpoints fail', async () => {
    ...
    await expect(
      discoverStories('http://localhost:6006', { fetchImpl: fetchMock }),
    ).rejects.toThrow(/Could not discover stories/);
    expect(fetchMock).toHaveBeenCalledTimes(3);
  });
